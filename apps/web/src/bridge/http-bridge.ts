...
async function renderTikz(source: string): Promise<TikzRenderResponse> {
  if (lastServerCapabilities?.supportsTikz !== true) {
    return { ok: false, error: 'TikZ rendering is not available in the web build yet.' }
  }
  try {
    return await jsonRequest<TikzRenderResponse>('/tikz', {
      method: 'POST',
      body: { source }
    })
  } catch (err) {
